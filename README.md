...
`/casino blackjack` で開始。36 スロット (4 行) のコンパクトなインベントリ UI でプレイ。
...
| 4 行目 (27-35) | アクションボタン: HIT=29 / STAND=31 / DOUBLE DOWN=33 (等間隔配置) |

手札は 7 枠の範囲 (1-7 / 19-25) に対して常に **中央寄せ** で描画されるため、
2 枚配布時は中央に固まり、ヒットで枚数が増えるほど左右に自然に広がります。
2 行目をあえて空けることでディーラー/プレイヤー間に余白を作り、背景パネルも
...
角丸パネル 1 枚 (176×106px、アクション行の直後で終わる高さ) をインベントリの
