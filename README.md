...
`/casino blackjack` で開始。45 スロット (5 行) のコンパクトなインベントリ UI でプレイ。
...
| 2 行目 (9-17) | (空欄 — ディーラー/プレイヤー間の余白) |
| 3 行目 (18-26) | プレイヤーの手札 (19-25 の範囲に中央寄せで配置) |
| 4 行目 (27-35) | (空欄 — プレイヤー行/アクション行間の余白) |
| 5 行目 (36-44) | アクションボタン: HIT=38 / STAND=40 / DOUBLE DOWN=42 (等間隔配置) |

手札は 7 枠の範囲 (1-7 / 19-25) に対して常に **中央寄せ** で描画されるため、
2 枚配布時は中央に固まり、ヒットで枚数が増えるほど左右に自然に広がります。
2・4 行目をあえて空けることで各セクション間に余白を作り、背景パネルも
アクション行の直後で終わるサイズにしているため、上部に内容が詰まって下部だけ
大きく余る、といった偏りが出ないようにしています。

カード・ボタンは `display.gui.scale` でそれぞれ 1.15 倍・1.1 倍に拡大表示していますが、
これは 1 スロットの高さ (18px) をわずかに超えるサイズです。空欄の行を挟まずに
隣接させると拡大分がタイトルバーや隣の行にはみ出して重なって見えるため、
拡大表示を使う場合は必ず空欄行を挟む構成にしています。
...
角丸パネル 1 枚 (176×124px、アクション行の直後で終わる高さ) をインベントリの
