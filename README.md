...
「余白行を挟んで手札を中央寄せ配置する」構成をそのまま踏襲し、コミュニティカード/自分の手札に割り当てています。

| 行 | 内容 |
|----|------|
| 1 行目 (0-8) | (空欄 — タイトル/場札行間の余白) |
| 2 行目 (9-17) | [9]=ディーラーの手札インジケーター(裏向き。ショーダウン時のみ表向き) [ コミュニティカード、10-16 の範囲に中央寄せで配置 ] [17]=同上 |
| 3 行目 (18-26) | (空欄 — 場札/手札行間の余白) |
| 4 行目 (27-35) | 自分のホールカード (2枚、28-34 の範囲に中央寄せ配置 → 常に30-31に着地) |
| 5 行目 (36-44) | (空欄 — 手札行/アクション行間の余白) |
| 6 行目 (45-53) | アクションボタン: BET=47 / CHECK・CALL=49 / FOLD=51 |

コミュニティカードは最大5枚を 10-16 の7スロット範囲に中央寄せで配置するため(ブラックジャックの
`placeHandCentered` と同じ技法)、中央寄せでは絶対に届かない両端のスロット(9・17)が常に空くことを
利用して、ここをディーラーの手札インジケーターとして再利用しています(手札が生きている間は裏向き、
ショーダウンでのみ表向きに反転)。ポット額・現在のストリート(PRE-FLOP/FLOP/TURN/RIVER)・結果は、
ブラックジャックと同様にインベントリタイトル直下に常時表示され、状態が変わるたびに GUI を再構築して
開き直しています。
